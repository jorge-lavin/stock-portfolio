...
class DividendsContainer extends React.Component<AllProps> {  
  ...
  public render() {
    ...
    const renderCurrency = (dividend: Dividend, currency: Currency) => 
      (dividend === undefined || currency === undefined) ? null : `${dividend.amount} ${currency.name}`
        ...
        <Table>
          ...
          <tbody>
            { loading ? null : dividends.entities.allIds.map((dividendId, index) => { 
              ...
              const currency = currencies.entities.byId[dividend.currencyId]
              return (
                <tr key={index}>
                  <td>{dividend.stockId}</td>
                  <td>{dividend.period}</td>
                  <td>{renderCurrency(dividend, currency)}</td>
                </tr> 
              )}) 
            }
          </tbody>
